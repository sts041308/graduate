  ...
  <div class="container" v-if="!showDetail">
    ...
    <el-table stripe v-loading="loading" :data="tableData">
      ...
      <el-table-column fixed="right" label="操作" width="400">
        <template slot-scope="scope">
          <el-button @click.prevent="handleDetail(scope.row)" type="success" plain size="small" round
            >查看简历</el-button
          >
          <el-button
            @click.prevent="onOpen(scope.row)"
            type="primary"
            plain
            size="small"
  ...
  methods: {
    ...
    async handelConfirm() {
      ...
      this.formData.time = moment(
        `${this.formData.interviewdate} ${this.formData.interviewTime}`,
        'YYYY-MM-DD HH:mm:ss',
      ).format()
